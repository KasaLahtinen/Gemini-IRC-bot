...
While these tools played a key role in the rapid prototyping and development of this bot, the overall design, integration, and final refinement were overseen and validated by human developers. **It's important to note that the majority of the code in this project was generated by AI**.
...
*   Uses threads to handle multiple channels.
*   Monitors resources usage.
*   Supports multithreading.
*   The bot has a monitor resources thread that displays the current resource usage.
*   The bot has the `hello_command` and `join_command` commented out. These commands are not working.
*   The bot uses `try/except` blocks to handle errors in the connection.
*   The bot logs resources usage in the `log_resource_usage` function.
